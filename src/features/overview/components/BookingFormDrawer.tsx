...
import { Drawer, Form, Input, Select, Button, InputNumber, Divider, Timeline } from 'antd';
import { User, Phone, CreditCard, DollarSign, Calendar } from 'lucide-react';
...
export const BookingFormDrawer: React.FC<BookingFormDrawerProps> = ({ isOpen, onClose, activeCellId }) => {
  ...
  const cellsToBook = activeCellId ? [activeCellId] : selectedCells;
    ...
    <Drawer
      ...
      footer={
         <div className="flex gap-4">
            <Button size="large" onClick={onClose} className="flex-1 font-medium">
              Cancel
            </Button>
            <Button 
               size="large" 
       ...
       <div className="p-6">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-8 shadow-sm">
             <div className="flex items-start gap-4 mb-4">
              ...
              <div className="bg-white rounded-lg border border-slate-100 p-4 mb-4 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)]">
                <Timeline 
                  className="pt-2 pl-1"
                      ...
                      return {
                        ...
                        children: (
                          <div className={`-mt-1.5 ${isLastBlockForCourt ? 'pb-6' : 'pb-2'}`}>
                            <div className="text-sm font-semibold text-slate-800 tracking-tight flex items-center gap-2">
                               {formatTimeSlot(block.start)} - {formatTimeSlot(endTimeObj)}
                               {block.count > 1 && (
                                 <span className="text-[10px] font-bold bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded-md leading-none border border-indigo-100 shadow-sm">
                                   {block.count} SLOTS
                                 </span>
                               )}
                            </div>
